...
#Region "Metodos Privados"
    ...
    Public Sub _prMostrarRegistro(_N As Integer)
        ...
        tbTransporte.Value = grVentas.GetValue("tatransp")
    ...
    Private Sub _prCargarVenta()
        ...
        With grVentas.RootTable.Columns("tafvcr")
            ...
            .Visible = False
        End With
        With grVentas.RootTable.Columns("fvanfac")
            .Width = 100
            .CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
            .Visible = True
            .Caption = "Nº FACTURA"
        ...
        With grVentas.RootTable.Columns("cliente")
            ...
            .Caption = "CLIENTE"
        End With
        With grVentas.RootTable.Columns("taobra")
            .Width = 50
            .CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Near
            .Visible = False
        End With
        ...
        With grVentas.RootTable.Columns("taice")
            ...
            .Visible = False
        End With
        With grVentas.RootTable.Columns("tatransp")
            .Width = 50
            .CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Near
            .Visible = False
        End With
    ...
    Private Sub _prCargarLotesDeProductos(CodProducto As Integer, nameProducto As String)
        ...
        End With
        'CAMBIAMOS EL NOMBRE DE LOTE A NRO. DE ORDEN
    ...
    Public Sub P_PonerTotal(rowIndex As Integer)
        If (rowIndex < grdetalle.RowCount) Then
            ...
            'Dim cant As Double = grdetalle.GetValue("tbcantu")
    ...
    Public Sub _prCalcularPrecioTotal()
        ...
        If (gb_FacturaIncluirICE = True) Then
            tbtotal.Value = grdetalle.GetTotal(grdetalle.RootTable.Columns("tbtotdesc"), AggregateFunction.Sum) - montodesc + tbIce.Value + tbTransporte.Value
        Else
            tbtotal.Value = grdetalle.GetTotal(grdetalle.RootTable.Columns("tbtotdesc"), AggregateFunction.Sum) - montodesc + tbTransporte.Value
        End If
    End Sub
    ...
    Public Sub InsertarProductosSinLote()
        ...
        If ((pos >= 0) And (Not existe)) Then
            ...
            CType(grdetalle.DataSource, DataTable).Rows(pos).Item("tbcantu") = 0
    ...
    Private Sub P_prImprimirFacturar(numi As String, impFactura As Boolean, grabarPDF As Boolean)
        ...
        Dim _Ds, _Ds1, _Ds2, _Ds3, _Ds4 As New DataSet
        ...
        End If
        _Ds4 = L_Reporte_FacturaVenta(numi)

        _Fecha = dtiFechaFactura.Value
        _Hora = _Ds4.Tables(0).Rows(0).Item("tahact").ToString
    ...
    End Sub
    Private Sub P_prReImprimirFacturar(numi As String, impFactura As Boolean, grabarPDF As Boolean)
        Dim _Fecha, _FechaAl As Date
        Dim _Ds, _Ds1, _Ds2, _Ds3, _Ds4 As New DataSet
        Dim _Autorizacion, _Nit, _Fechainv, _Total, _Key, _Cod_Control, _Hora,
            _Literal, _TotalDecimal, _TotalDecimal2 As String
            ...
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                If (lote <> String.Empty) Then
                    If (cant <= iccven) Then
                        ...
                        grdetalle.SetValue("tbcantu", cantu)
                        grdetalle.SetValue("tbccaja", ccaja)
                        grdetalle.SetValue("tbcantc", cantc)
                        grdetalle.SetValue("tbcmin", cant)
                        grdetalle.SetValue("tbptot2", dt.Rows(i).Item("pcosto") * cantu)
                    ...
                    grdetalle.SetValue("codigo", numimanual)
    ...
    Public Sub _prPedirLotesProducto(ByRef lote As String, ByRef FechaVenc As Date, ByRef iccven As Double, CodProducto As Integer, nameProducto As String, cant As Integer)
        ...
        listEstCeldas.Add(New Modelo.Celda("iclot", True, "NRO. ORDEN", 150))
        listEstCeldas.Add(New Modelo.Celda("icfven", True, "FECHA ORDEN", 180, "dd/MM/yyyy"))
        listEstCeldas.Add(New Modelo.Celda("iccven", True, "Stock".ToUpper, 150))
        Dim ef = New Efecto
        ef.tipo = 3
        ef.dt = dt
...
#Region "Eventos Formulario"
    ...
    Private Sub grdetalle_EditingCell(sender As Object, e As EditingCellEventArgs) Handles grdetalle.EditingCell
        If (_fnAccesible()) Then
            ...
            If (e.Column.Index = grdetalle.RootTable.Columns("yfcbarra").Index Or
                e.Column.Index = grdetalle.RootTable.Columns("tbcantu").Index Or
...
salirIf:
    ...
    Private Sub ponerProducto(codigo As String)
        ...
        If (fila.Count > 0) Then
            ...
            'CType(grdetalle.DataSource, DataTable).Rows(pos).Item("tbcmin") = 1
            CType(grdetalle.DataSource, DataTable).Rows(pos).Item("tbcantu") = 1
            If (gb_FacturaIncluirICE) Then
    ...
    Private Sub grdetalle_CellValueChanged(sender As Object, e As ColumnActionEventArgs) Handles grdetalle.CellValueChanged
        ...
        If (e.Column.Index = grdetalle.RootTable.Columns("tbcantu").Index) Then
            If (Not IsNumeric(grdetalle.GetValue("tbcantu")) Or grdetalle.GetValue("tbcantu").ToString = String.Empty) Then
                ...
                CType(grdetalle.DataSource, DataTable).Rows(pos).Item("tbccaja") = grdetalle.GetValue("tbcantu") / conv
                ...
                CType(grdetalle.DataSource, DataTable).Rows(pos).Item("tbcantc") = CjReal
                CType(grdetalle.DataSource, DataTable).Rows(pos).Item("tbcantc") = CjReal * conv
            ...
            Else
                If (grdetalle.GetValue("tbcantu") > 0) Then
                    ...
                    Else
                        CType(grdetalle.DataSource, DataTable).Rows(pos).Item("tbccaja") = grdetalle.GetValue("tbcantu") / conv
                        grdetalle.SetValue("tbccaja", grdetalle.GetValue("tbcantu") / conv)
                        Dim CjReal As Double = (grdetalle.GetValue("tbccaja"))
                        grdetalle.SetValue("tbcantc", CjReal)
                        grdetalle.SetValue("tbcmin", CjReal * conv)
                        CType(grdetalle.DataSource, DataTable).Rows(pos).Item("tbcantc") = CjReal
                        CType(grdetalle.DataSource, DataTable).Rows(pos).Item("tbcantc") = CjReal * conv
                    ...
                    If (grdetalle.GetValue("tbcmin") > stock) Then
                        Dim img As Bitmap = New Bitmap(My.Resources.mensaje, 50, 50)
                        ToastNotification.Show(Me, "La cantidad de la venta introducida = " + Str(cantidad) + "  no debe ser mayor al del stock" & vbCrLf &
    ...
    Private Sub grdetalle_CellEdited(sender As Object, e As ColumnActionEventArgs) Handles grdetalle.CellEdited
        ...
        '            Dim cant As Integer = grdetalle.GetValue("tbcantu")
    ...
    Private Sub btnImprimir_Click(sender As Object, e As EventArgs) Handles btnImprimir.Click
        If (Not _fnAccesible()) Then
            ...
            P_prReImprimirFacturar(tbCodigo.Text, True, True)
        End If
    End Sub
    ...
    Private Sub tbProforma_KeyDown(sender As Object, e As KeyEventArgs) Handles tbProforma.KeyDown
        If (_fnAccesible()) Then
            If e.KeyData = Keys.Control + Keys.Enter Then
                ...
                listEstCeldas.Add(New Modelo.Celda("paobra", False, "", 50))
                listEstCeldas.Add(New Modelo.Celda("oanomb", False, "OBRA", 220))
                listEstCeldas.Add(New Modelo.Celda("padesc", False, "DESCUENTO", 100))
                listEstCeldas.Add(New Modelo.Celda("patransp", False, "TRANSPORTE", 220))
                listEstCeldas.Add(New Modelo.Celda("total", True, "TOTAL".ToUpper, 120))
                listEstCeldas.Add(New Modelo.Celda("paalm", False, "", 50))
                Dim ef = New Efecto
                ef.tipo = 3
                ef.dt = dt
                Modelo.MGlobal.SeleccionarCol = 5
                ef.SeleclCol = 2
                ef.listEstCeldas = listEstCeldas
                ef.alto = 50
                ef.ancho = 350
        ...
        If (tbTransporte.Focused) Then
            ...
            If (Not tbTransporte.Text = String.Empty) Then
                ...
                Else
                    ...
                    tbtotal.Value = grdetalle.GetTotal(grdetalle.RootTable.Columns("tbtotdesc"), AggregateFunction.Sum) - tbMdesc.Value + transporte
                End If
